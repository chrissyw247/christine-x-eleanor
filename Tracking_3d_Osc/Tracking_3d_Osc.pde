...
void drawSkeleton(int userId) {
 ...
 //PVector to get x, y, z, of any indivitual joint, in this case the head!
 PVector test = getJoint(userId, SimpleOpenNI.SKEL_HEAD);
 keyboard(test);
...
void keyboard(PVector joint) {
   firstTrig.checkCollision(joint);
 
 //Checks if the target area is being triggered and performs different actions
 if (firstTrig.triggered == true){
   tint(250, 0, 0);
   //if (state == "not triggered"){
   //  state = "triggered";
   int temp = int(map(joint.x, Constants.X1, Constants.X2, 0.5, 14.4));
   message = Integer.toString(temp);
//   println("message: " + message + " state: " + state);

  if (!state.equals(message)){
    state = message;

//write helper function to map the x coordinate onto keyboard
